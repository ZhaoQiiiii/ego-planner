...
std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points,
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
      ...
      int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point"
    ...
    if (segment_ids[i].second - segment_ids[i].first == 1) {
      Eigen::Vector3d ctrl_pts_law(cps_.points.col(segment_ids[i].second) - cps_.points.col(segment_ids[i].first)),
          intersection_point;
  ...
  for (int i = 0; i < q.cols() - 1; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      if (vi(j) > max_vel_ + demarcation) {
        ...
        cost +=
            (ar * diff * diff + br * diff + cr) * ts_inv3; // multiply ts_inv3 to make vel and acc has similar magnitude
